...
if (!IS_LOGGED && empty($channel_id)) {
	...
	foreach ($list as $key => $list_video) {
		...
		if (isset($video->video_id)) {
	        ...
	        $count = $db->where('user_id', $pt->user->id)->where('list_id', $list_video->list_id)->getValue(T_PLAYLISTS, 'count(*)');
	        if ($count > 0) {
	        	$list_video->count = $db->where('user_id', $pt->user->id)->where('list_id', $list_video->list_id)->getValue(T_PLAYLISTS, 'count(*)');
	        }
	    }
